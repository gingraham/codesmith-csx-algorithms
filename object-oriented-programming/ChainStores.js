...
class Chain {
    ...
    openStore(owner, city){
        const franchise = new Franchise(owner, city)
        this.locations.push(franchise);
        this.totalStores++;
    }

    closeStore(city){
        let franchiseIndex = this.locations.indexOf(this.locations.find(location => location.city === city))
        if(franchiseIndex === - 1){
            console.log(`${this.franchise} doesn't have a store in ${city}.`)
        }else{
            console.log(`${this.franchise} closed the store in ${city}.`)
        }
    }
}
